...
#使用return_sequences=True增加lstm的层数
#由于使用了Dropout，所以拟合会相对慢一些。
model = Sequential()
...
model.fit(x_train, y_train,batch_size=16, epochs=500,validation_data=(x_test,y_test),shuffle=True,callbacks=[])
