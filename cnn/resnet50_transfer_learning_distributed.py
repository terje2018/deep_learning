...
base_model=ResNet50V2(include_top=False, weights='imagenet',pooling='max')
x=base_model.output
x=Dropout(0.3)(x)
...
for i in range(1,5,1):
    ...
    model.fit(x=train_imgs_scaled, y=train_labels_list,
              validation_data=(validation_imgs_scaled, validation_labels_list), batch_size=32, epochs=5, verbose=1,
              shuffle=True)
    print(model.layers[-2].weights)
